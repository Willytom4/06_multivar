...
- Based on Y (outcome) existence:
    - Supervised  - if you have a y variable (telling the algorithm to help find how you arrived at your y)
    - Unsupervised  - if you do not have a y variable
...
- Model estimates (slope) magnitude and direction (+ or -) can change for multicollinear variables compared to a non-multicollinear model.  
...
lm_dayl.apr <- lm(strength_gtex ~ mean_dayl.s_Apr,
                  data = weather) %>%
  tidy() %>% #produce the output in a table format
...
lm_dayl.apr %>%
  ...
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) #dodge helps to adjust the axis label
...
weather_n
#PCA does not take a y variable and categorical variables, so we need to remove y and non-numerical input
...
fviz_eig(mod_pca,
         addlabels = T)
#produce dimensions that are uncorrelated and to reduce dimensions : why we use PCA Exam
#scree plot helps to visualize the PCA
```
...
PC1 ~ sum_precip_oct x coefficient +
      mean_daylength_jan x coefficient
